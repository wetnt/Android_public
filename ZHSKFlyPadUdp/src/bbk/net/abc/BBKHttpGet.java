...
public class BBKHttpGet {
	...
	// 此过程必须放在新线程中：see81192154:
	{
		try {
			// -------------------------------------------------------------------
			if (debugKey) {
				d.s("BBKHttpGet.BBKHttpGetUrlNew.BackStrCode = " + strCode, false, false);
				d.s("BBKHttpGet.BBKHttpGetUrlNew.proxy.sort.out = " + proxy + "," + sort + "," + timeout, false, false);
				d.s("BBKHttpGet.BBKHttpGetUrlNew.strUrl == " + strUrl, false, false);
			}
			// -------------------------------------------------------------------
			...
			if (conn == null) {
				if (debugKey)
					d.s("BBKHttpGet.BBKHttpGetUrlNew.conn == null", false, false);
				return null;
			}
			// -------------------------------------------------------------------
			...
			// -------------------------------------------------------------------
			if (debugKey)
				d.s("BBKHttpGet.BBKHttpGetUrlNew.strResult =" + allstr, false, false);
			// -------------------------------------------------------------------
			return allstr;
			// -------------------------------------------------------------------
		} catch (IOException e) {
