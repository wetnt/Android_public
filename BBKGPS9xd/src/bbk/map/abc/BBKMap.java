...
public class BBKMap {
	...
	// ####################################################################################
	// ====================================================================================
	public static float TileZoom = 2.0f;
	private int TileSize = (int) (256 * TileZoom);
	public boolean MapRegKey = true;// true:国内,false:国外
	...
	private void MapBmpsCalcs() {
		...
		// ---------------------------------------------------------
		PicX = Math.round(PixJXF / TileSize);
		ModX = PixJXF % TileSize;
		PicY = Math.round(PixWYF / TileSize);
		ModY = PixWYF % TileSize;
		// ---------------------------------------------------------
	}
	...
	private void MapDraw() {
		...
		// ------------------------------------------------------------------------
		int leftR = (MapWx - ModX) / TileSize + 1, rigtR = (MapWx - (TileSize - ModX)) / TileSize + 1;
		int topsR = (MapHy - ModY) / TileSize + 1, botmR = (MapHy - (TileSize - ModY)) / TileSize + 1;
		boolean ptu = false;
		// ------------------------------------------------------------------------
		for (int y = -topsR; y <= botmR; y++) {
			for (int x = -leftR; x <= rigtR; x++) {
				...
				if (ptu) {
					...
					mapPts.SetPt(pix, piy, mapzm, maptype);
					// --------------------------------------------------------
					pixx = MapWx + x * TileSize - ModX;
					piyy = MapHy + y * TileSize - ModY;
					// --------------------------------------------------------
					// BBKDebug.d(pix+"_"+piy+"_"+mapzm+"_"+maptype,false,false);
					BBKMapImage.Pic = myBBD.ReadAllDirsPic(mapPts, maptype < 3);
					// --------------------------------------------------------
					if (BBKMapImage.Pic != null) {
						BBKMapImage.BBKMapDrawImage256(BBKMapImage.Pic, pixx, piyy, TileZoom, true);
					} else {
						BBKMapImage.BBKMapDrawImage256(BBKMapImage.Lgo, pixx, piyy, TileZoom, false);
					}
					// ------------------------------------------------------------
				}
			}
	...
	public void BBKViewCenterTemp(int cx, int cy) {
		...
		// -------------------------------------------------------------------------
		int xx = 12;
		int x0 = cx - aw, y0 = cy - ah, x1 = cx + aw, y1 = cy - ah;
		int x2 = cx + aw, y2 = cy + ah, x3 = cx - aw, y3 = cy + ah;
		// -------------------------------------------------------------------------
